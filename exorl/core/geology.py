...
VOLCANIC_GAS = {
    "plate_tectonics": {"H2O": 0.79, "CO2": 0.12, "SO2": 0.06, "N2": 0.02, "H2S": 0.01},
    "stagnant_lid": {"CO2": 0.60, "H2O": 0.25, "SO2": 0.10, "N2": 0.05},
    "hot_spot": {"CO2": 0.55, "H2O": 0.35, "SO2": 0.08, "N2": 0.02},
...
class GeologyModel:
    ...
    def __init__(self, planet, star=None, orbital_dist_m: float = None):
        ...
        self.orbital_dist = orbital_dist_m or getattr(
            planet, "orbital_distance_m", None
        )
    ...
    def _estimate_T_eq(self) -> float:
        ...
        try:
            ...
            pass
        return float(
            getattr(getattr(self.planet, "atmosphere", None), "surface_temp", 255)
            * 0.88
        )
    ...
    def apply_to_planet(
        ...
        correct_composition: bool = False,
    ) -> dict:
        """
