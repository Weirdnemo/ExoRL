...
def synodic_period_days(period1_days, period2_days):
    ...
    if abs(period1_days - period2_days) < 1e-6:
        return float("inf")
...
class LaunchWindow:
    ...
    @property
    def vinf_dep_m_s(self):
        return self.vinf_dep_km_s * 1e3

    @property
    def vinf_arr_m_s(self):
        return self.vinf_arr_km_s * 1e3
    ...
    def total_dv(
        ...
        return patched_conic_budget(
            ...
            self.vinf_dep_m_s,
            self.vinf_arr_m_s,
            self.departure_planet,
            self.arrival_planet,
        )

    def report(self):
        return (
            f"=== Launch window: {self.departure_planet} -> {self.arrival_planet} ===\n"
...
class PorkchopData:
    ...
    def compute(
        ...
        c3g = np.full((nd, na), np.nan)
        vg = np.full((nd, na), np.nan)
        tg = np.full((nd, na), np.nan)
        vl = np.zeros((nd, na), dtype=bool)
        solver = LambertSolver(mu_star)
        for i, td in enumerate(departure_days):
            ...
            for j, ta in enumerate(arrival_days):
                ...
                vl[i, j] = True
        return cls(
            np.asarray(departure_days),
            np.asarray(arrival_days),
            c3g,
            vg,
            tg,
            vl,
    ...
    def best_window(
        self, max_c3=30.0, max_vinf_arr=8.0, min_tof_days=60.0, metric="c3"
    ):
        mask = (
            self.valid
            & (self.c3 <= max_c3)
            & (self.vinf_arr <= max_vinf_arr)
            & (self.tof >= min_tof_days)
        )
        if not mask.any():
            return None
        score = np.where(
            ...
            if metric == "vinf_arr"
            else self.c3 + self.vinf_arr**2,
            np.inf,
        )
        i, j = np.unravel_index(np.argmin(score), score.shape)
        return LaunchWindow(
            float(self.departure_days[i]),
            float(self.arrival_days[j]),
            float(self.tof[i, j]),
            float(self.c3[i, j]),
            float(math.sqrt(self.c3[i, j])),
            float(self.vinf_arr[i, j]),
            self.departure_planet,
            self.arrival_planet,
        )
    ...
    def cost_at(self, dep_day, arr_day):
        c3, va, tof, ok = compute_transfer(
            self.departure_radius_m, self.arrival_radius_m, dep_day, arr_day
        )
        if not ok:
            return None
        return LaunchWindow(
    ...
    def min_c3(self):
        v = self.c3[self.valid]
        return float(v.min()) if len(v) > 0 else float("nan")

    def min_vinf_arr(self):
        v = self.vinf_arr[self.valid]
        return float(v.min()) if len(v) > 0 else float("nan")

    def summary(self):
        n = int(self.valid.sum())
        return (
            f"Porkchop: {self.departure_planet} -> {self.arrival_planet}\n"
...
class LaunchDecisionSpace:
    ...
    def __init__(
        ...
        self.r_dep = dep_radius_m
        self.r_arr = arr_radius_m
        self.n_dep = n_dep
        self.n_arr = n_arr
        self.min_tof = min_tof_days
        self.max_tof = max_tof_days
        self.departure_days = np.linspace(
            window_start_day, window_start_day + window_duration_days, n_dep
        )
    ...
    def cost(self, dep_idx, arr_idx):
        di = max(0, min(self.n_dep - 1, dep_idx))
        ai = max(0, min(self.n_arr - 1, arr_idx))
    ...
    def observation(self, dep_idx, arr_idx):
        ...
        if not cost["valid"]:
            return np.array(
                [dep_idx / self.n_dep, arr_idx / self.n_arr, 1.0, 1.0, 1.0, 0.0],
                dtype=np.float32,
            )
        return np.array(
            [
                dep_idx / self.n_dep,
                arr_idx / self.n_arr,
                min(cost["c3"] / 30.0, 1.0),
                min(cost["vinf_arr"] / 8.0, 1.0),
                min(cost["tof_days"] / self.max_tof, 1.0),
                1.0,
            ],
            dtype=np.float32,
        )
    ...
    def reward(self, dep_idx, arr_idx, max_c3=30.0, max_vinf_arr=8.0):
        cost = self.cost(dep_idx, arr_idx)
        if not cost["valid"]:
            return -1.0
        return -0.5 * (
            min(cost["c3"] / max_c3, 1.0) + min(cost["vinf_arr"] / max_vinf_arr, 1.0)
        )

    def summary(self):
        bi, bj = self.best_action()
        best = self.cost(bi, bj)
        syn = synodic_period_days(
            orbital_period_days(self.r_dep), orbital_period_days(self.r_arr)
        )
        return (
            f"LaunchDecisionSpace\n"
