...
class TransitSignal:
    ...
    def report(self) -> str:
        ...
        detect_str = (
            " + ".join(detect_s) if detect_s else "challenging with current instruments"
        )
...
def geometric_transit_probability(
    star_radius_m: float, orbital_distance_m: float, eccentricity: float = 0.0
) -> float:
    """
...
def minimum_detectable_mass_kg(
    K_rms_m_s: float, star_mass_kg: float, orbital_period_s: float
) -> float:
    """
...
def photon_noise_floor_ppm(
    star_apparent_magnitude: float,
...
def transit_snr(
    ...
    systematic_floor_ppm: float = 10.0,
) -> float:
    """
    ...
    sigma_per_exp = math.sqrt(
        photon_noise_floor_ppm(star_magnitude_V, telescope_aperture_m, exposure_time_s)
        ** 2
        + read_noise_ppm**2
    )
...
def transmission_spectral_feature_depth_ppm(
    planet_radius_m: float,
    star_radius_m: float,
...
def transmission_spectrum(planet, star=None, orbital_distance_m: float = None) -> dict:
    """
    ...
    # Mean molar mass
    mm_g_mol = sum(MOLAR_MASS.get(sp, 28.0) * f for sp, f in composition.items())
...
def characterise_observations(
    ...
    dur_s = transit_duration(planet.radius, star.radius, orbital_distance_m, T_orb, b)
    ingress_s = transit_ingress_duration(
        planet.radius, star.radius, orbital_distance_m, T_orb, b
    )
    ...
    # S/N
    snr1 = transit_snr(depth_ppm, star_magnitude_V, dur_s / 3600, telescope_aperture_m)
    ...
    mm_g_mol = (
        sum(MOLAR_MASS.get(sp, 28.0) * f for sp, f in comp.items()) / total
        if total > 0
        else 28.0
    )
    ...
    tsm = transmission_spectroscopy_metric(
        ...
        T_eq,
        star_magnitude_J,
        mean_molar_mass_g_mol=mm_g_mol,
    )
