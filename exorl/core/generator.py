...
def preset_earth() -> Planet:
    return Planet(
        ...
        terrain=TerrainConfig(
            terrain_type=TerrainType.OCEANIC, max_elevation=8_848, min_elevation=-11_000
        ),
        ...
        oblateness=OblatenessConfig(
            enabled=True, J2=1.08263e-3, flattening=1 / 298.257
        ),
        moons=MoonConfig(
            enabled=True, count=1, mass_fraction=0.01230, orbit_radius=384_400e3
        ),
    )
...
def preset_mars() -> Planet:
    return Planet(
        ...
        atmosphere=AtmosphereConfig(
            **ATMOSPHERE_PRESETS[AtmosphereComposition.CO2_THIN],
            composition=AtmosphereComposition.CO2_THIN,
        ),
        terrain=TerrainConfig(
            terrain_type=TerrainType.CRATERED,
            max_elevation=21_900,
            min_elevation=-7_200,
        ),
        magnetic_field=MagneticFieldConfig(enabled=False),
        oblateness=OblatenessConfig(enabled=True, J2=1.960e-3, flattening=1 / 169.8),
        moons=MoonConfig(
            enabled=True, count=2, mass_fraction=1.8e-8, orbit_radius=9_376e3
        ),
    )
...
def preset_venus() -> Planet:
    return Planet(
        ...
        atmosphere=AtmosphereConfig(
            **ATMOSPHERE_PRESETS[AtmosphereComposition.CO2_THICK],
...
def preset_titan() -> Planet:
    return Planet(
        ...
        atmosphere=AtmosphereConfig(
            **ATMOSPHERE_PRESETS[AtmosphereComposition.METHANE],
...
class PlanetGenerator:
    ...
    def generate(
        ...
        if atmosphere_enabled:
            ...
            no_atm_prob = (
                0.3 if (allow_no_atmosphere and radius < 0.4 * R_EARTH) else 0.05
            )
            if allow_no_atmosphere and rng.random() < no_atm_prob:
                atm = AtmosphereConfig(enabled=False)
            else:
                comp = atmosphere_composition or rng.choice(
                    list(AtmosphereComposition)[1:]
                )  # skip NONE
                base = dict(
                    ATMOSPHERE_PRESETS.get(
                        comp, ATMOSPHERE_PRESETS[AtmosphereComposition.EARTH_LIKE]
                    )
                )
        ...
        if magnetic_field_enabled:
            ...
            mag = MagneticFieldConfig(
                ...
                radiation_belt_enabled=(
                    strength != MagneticFieldStrength.NONE and rng.random() < 0.5
                ),
