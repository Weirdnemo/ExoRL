...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--eval-freq", type=int, default=10_000, help="Evaluate every N steps"
    )
    parser.add_argument(
        "--eval-episodes", type=int, default=20, help="Episodes per evaluation"
    )
    parser.add_argument("--tag", default="run", help="Experiment name (output folder)")
    ...
    parser.add_argument(
        "--lite",
        action="store_true",
        help="Fast training mode: simplified planets + no science stack (implies --no-science)",
    )
    parser.add_argument(
        "--pretrain",
        default=None,
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Defer RL imports so `pip install -e .` still lets you import this module.
    ...
    def make_env(seed_offset: int = 0):
        def _init():
            ...
            if args.mode == "fixed":
                ...
                if args.lite:
                    raise SystemExit(
                        "Lite mode does not support --mode curriculum "
                        "(habitability-based curriculum needs the science stack)."
                    )
    ...
    class MetricsCallback(BaseCallback):
        def __init__(self, out_dir: Path, eval_freq: int, eval_episodes: int, eval_env):
        ...
        def _on_training_start(self):
            ...
            print(f"\n{'─' * 80}")
            print(
                f"  {'step':>8s}  {'reward':>8s}  {'success':>7s}  "
        ...
        def _run_eval(self):
            ...
            self._csv.write(
                f"{self.num_timesteps},{mean_r:.4f},{std_r:.4f},{succ_r:.4f},"
                f"{fuel_eff:.4f},{hab_b:.4f},{p_hab:.4f}\n"
            )
            self._csv.flush()
        ...
        def _on_training_end(self):
            ...
            print(f"{'─' * 80}")
            print(
                f"  Training complete: {total_time:.0f}s  "
                f"({self.num_timesteps / max(total_time, 1):.0f} fps avg)"
            )
    ...
    print(
        f"SAC policy: {n_params:,} parameters  hidden={args.hidden}×2  obs={obs_dim}  act=3"
    )

    if args.pretrain:
        from pathlib import Path as _Path

        if _Path(args.pretrain).exists():
            import torch
    ...
    print(
        f"Training:   {args.steps:,} steps  eval_freq={args.eval_freq:,}  seed={args.seed}"
    )
    print(
        f"Mode:       {args.mode}"
        + (f"  planet={args.planet}" if args.mode == "fixed" else "")
        + ("  lite=on" if args.lite else "")
    )
    ...
    eval_results = {
        ...
        "steps_per_second": args.steps / max(train_time, 1e-9),
    }
    ...
    print(
        f"  Train time:      {train_time:.0f}s  ({eval_results['steps_per_second']:.0f} fps)"
    )
    print(f"{'═' * 60}")

    # Optional learning curve figure
    try:
        import matplotlib
        ...
        steps_l, rewards_l, success_l, fuel_l = [], [], [], []
        ...
        if len(steps_l) > 1:
            ...
            fig.subplots_adjust(
                left=0.07, right=0.97, top=0.87, bottom=0.14, wspace=0.35
            )
            ...
            fig.savefig(str(out_dir / "learning_curve.png"), dpi=150, bbox_inches="tight")
            plt.close(fig)
