...
# ── science imports ───────────────────────────────────────────────────────────
from core import (
    PRESETS, PlanetGenerator,
    AtmosphereComposition, TerrainType,
    InteriorConfig, interior_from_bulk_density,
    Star, STAR_PRESETS, star_sun, star_proxima_centauri,
    star_trappist1, star_tau_ceti, star_kepler452, star_eps_eridani, AU,
    SpectralType,
)
from core.atmosphere_science import (
    MultiLayerAtmosphere, JeansEscape, GreenhouseModel,
    analyse_atmosphere, STANDARD_COMPOSITIONS,
)
from core.habitability import (
    assess_habitability, size_class, composition_class,
)
...
from core.ground_track import (
    propagate_ground_track, compute_coverage_map, find_passes,
)
from core.surface_energy import (
    compute_insolation_map, compute_temperature_map, surface_energy_balance,
    permanent_shadow_latitude_deg,
)
from core.tidal import (
    TidalHeating, TidalLocking, RocheLimit, analyse_tidal,
)
from core.mission import (
    orbital_insertion_dv, plan_aerobraking, build_mission_dv_budget,
    GravityAssist, porkchop_data,
)
from visualization import (
    plot_planet_cross_section, plot_atmosphere_profile,
    apply_journal_style, save_figure,
    WONG, W_BLUE, W_RED, W_GREEN, W_ORANGE, W_BLACK,
    W_PINK, W_SKY, W_YELLOW,
    FT, FL, FK, FA, FG, LW, LW2,
)
...
earth.star_context = sun;  earth.orbital_distance_m = 1.000 * AU
mars.star_context  = sun;  mars.orbital_distance_m  = 1.524 * AU
venus.star_context = sun;  venus.orbital_distance_m = 0.723 * AU
moon.star_context  = sun;  moon.orbital_distance_m  = 1.000 * AU
titan.star_context = sun;  titan.orbital_distance_m = 9.537 * AU
...
gs1  = gridspec.GridSpec(2, 5, figure=fig1,
...
PIE_COLS = {"iron_solid": W_RED, "iron_liquid": "#E86060",
            "iron_sulfide": "#D07070", "perovskite": "#C4956A",
            "silicate_mix": "#A88060", "olivine": "#B09878",
            "basalt": "#8A7060", "granite": "#BCA898",
            "water_ice": W_SKY, "liquid_water": W_BLUE,
            "high_pressure_ice": "#80B8D0", "other": "#AAAAAA"}
...
for i, planet in enumerate(ALL_PLANETS):
    ...
    if planet.interior and planet.interior.enabled and planet.interior.layers:
        ...
        ax_pie.text(0.5, 0.5, "no\nmodel", ha="center", va="center",
                    fontsize=FA, color="#999999", transform=ax_pie.transAxes)
        ax_pie.axis("off")
    ax_pie.set_aspect("equal")

fig1.suptitle("Solar system analogues — cross-sections & interior structure",
...
ax.set_ylabel("MoI factor  C/(MR²)", fontsize=FL); ax.set_ylim(0, 0.45)
ax.set_title("(a) Moment of inertia factor", fontsize=FT, fontweight="bold", pad=3)
ax.legend(fontsize=FG-1, framealpha=0.9); ax.tick_params(labelsize=FK)
for b, v in zip(bars, moi_vals):
    ax.text(b.get_x() + b.get_width()/2, v + 0.003, f"{v:.3f}",
            ha="center", va="bottom", fontsize=FA)
...
ax.set_title("(c) Derived surface magnetic field", fontsize=FT, fontweight="bold", pad=3)
ax.tick_params(labelsize=FK)
for b, v in zip(bars3, b_vals):
    ax.text(b.get_x() + b.get_width()/2, max(v, 0) + 0.5, f"{v:.1f}",
            ha="center", va="bottom", fontsize=FA)
...
fig2.suptitle("Interior model: physically derived planetary properties",
              fontsize=FT+1, fontweight="bold")
...
stars = [sun, star_tau_ceti(), star_kepler452(), star_eps_eridani(),
         star_proxima_centauri(), star_trappist1()]
...
for i, (star, sc) in enumerate(zip(stars, star_colors)):
    ...
    ax3a.barh(y, hz_out_opt - hz_in_opt, left=hz_in_opt, height=0.55,
              color=sc, alpha=0.25)
    # Conservative HZ (solid)
    ax3a.barh(y, hz_out - hz_in, left=hz_in, height=0.55,
              color=sc, alpha=0.80, label=star.name)
    # Reference planets
    for pname, pdist in [("Venus", 0.723), ("Earth", 1.0), ("Mars", 1.524)]:
        if hz_in_opt * 0.5 <= pdist <= hz_out_opt * 2:
            ...
            if i == 0:
                ax3a.text(pdist, y + 0.35, pname, ha="center",
                          fontsize=FA-0.5, color=W_BLACK)
...
ax3a.set_title("(a) Habitable zones (conservative + optimistic)",
               fontsize=FT, fontweight="bold", pad=3)
ax3a.set_xlim(0, 2.2)
...
bars_x = ax3b.barh(list(range(len(stars))),
                   [s.xuv_luminosity / s.luminosity * 1e3 for s in stars],
                   color=star_colors, edgecolor="white", height=0.6)
...
ax3b.set_title("(b) XUV/bolometric luminosity ratio",
               fontsize=FT, fontweight="bold", pad=3)
ax3b.axvline(1.0, color=W_BLACK, lw=LW2, ls="--", label="Sun reference")
ax3b.legend(fontsize=FG-1)

fig3.suptitle("Stellar environments: habitable zones and high-energy radiation",
              fontsize=FT+1, fontweight="bold")
...
for col, (planet, pname, pc) in enumerate(atm_planets):
    ...
    ax2.plot(np.array(rhos)*1000, alts/1e3, color=pc, lw=LW, ls="--",
             label="ρ (g/m³)")
    ...
    ax.tick_params(labelsize=FK); ax2.tick_params(labelsize=FK)
...
for si, sp in enumerate(species_show):
    ...
    for planet in ALL_PLANETS:
        ...
        T_exo = planet.atmosphere.surface_temp * 2.5 if planet.atmosphere.enabled else 500
        lam = JeansEscape.lambda_parameter(sp, v_esc, T_exo)
        lams.append(min(lam, 200))
    ax4d.bar(x + si*w, lams, width=w, label=sp,
             color=WONG[si % len(WONG)], edgecolor="white")
ax4d.axhline(20, color=W_BLACK, lw=LW2, ls="--", label="λ=20 retention limit")
ax4d.set_xticks(x + w*2); ax4d.set_xticklabels(planet_names, fontsize=FK, rotation=20)
ax4d.set_ylabel("Jeans parameter λ", fontsize=FL)
ax4d.set_title("(d) Jeans escape parameter", fontsize=FT, fontweight="bold", pad=3)
ax4d.legend(fontsize=FG-1, ncol=2, framealpha=0.9); ax4d.tick_params(labelsize=FK)
...
for pi, (planet, pcolor) in enumerate(zip(ALL_PLANETS, WONG)):
    ...
    if aa.get("enabled"):
        ...
        ax4f.plot([t_eq], [t_sf], "o", color=pcolor, ms=7, zorder=5,
                  label=planet.name)
        ax4f.annotate(planet.name, (t_eq, t_sf), textcoords="offset points",
                      xytext=(5, 3), fontsize=FA-0.5, color=pcolor)
...
ax4f.legend(fontsize=FG-1, framealpha=0.9); ax4f.tick_params(labelsize=FK)

fig4.suptitle("Atmospheric science: structure, escape, and greenhouse physics",
              fontsize=FT+1, fontweight="bold")
...
super_e = gen.generate("Super-Earth", atmosphere_enabled=True,
                        radius_range=(1.2*6.371e6, 1.6*6.371e6),
                        density_range=(4500, 5500),
                        magnetic_field_enabled=True,
                        oblateness_enabled=True, moons_enabled=False)
...
assessments = [assess_habitability(p, p.star_context, p.orbital_distance_m)
               for p in hab_planets]

factor_names = list(assessments[0].factors.keys())
factor_short = ["Stellar\ntype", "Stellar\nage", "HZ\npos.", "Surf.\ntemp",
                "Liq.\nwater", "Atm.\nretain", "Mag.\nshield",
                "Tidal\nlock", "Interior", "Size"]
...
for pi, (ha, hc) in enumerate(zip(assessments, ha_colors)):
    ...
    grade_col = {
        "A": W_GREEN, "B": W_BLUE, "C": W_ORANGE, "D": W_RED, "F": "#888888"
    }.get(ha.grade, W_BLACK)
    ax.set_title(
        f"{ha.planet_name}\n"
        f"score={ha.overall_score:.2f}  grade={ha.grade}",
        fontsize=FA+0.5, fontweight="bold", pad=8, color=grade_col
    )

fig5.suptitle("Habitability assessment: ten-factor radar comparison",
              fontsize=FT+1, fontweight="bold")
...
for inc, ls in zip(incs, linestyles):
    rates = [J2Analysis.nodal_precession_rate_deg_day(
...
ax.legend(fontsize=FG-1, framealpha=0.9, ncol=2); ax.tick_params(labelsize=FK)
...
frz_eccs = [FrozenOrbit.frozen_eccentricity(earth, earth.radius + 500e3,
             math.radians(i)) for i in inc_range]
...
ax.set_title("(c) Frozen orbit eccentricity (500 km)", fontsize=FT, fontweight="bold", pad=3)
ax.legend(fontsize=FG); ax.tick_params(labelsize=FK)
...
for bv, bl, lss in zip(b_vals, b_labels, ["-", "--", "-."]):
    lifetimes = [DragLifetime.lifetime_years(earth, h*1e3, 1000, bv)
                 for h in alts3]
...
ax.set_title("(d) Atmospheric drag lifetime (Earth)", fontsize=FT, fontweight="bold", pad=3)
ax.legend(fontsize=FG-1, framealpha=0.9); ax.tick_params(labelsize=FK)

# (e) Station-keeping ΔV per year vs altitude
ax = axes6[1, 1]; _ax(ax)
alts4 = np.linspace(250, 700, 100)
sk_vals = [StationKeeping.total_annual_budget(earth, h, 98.0, 100, T_earth_year
               )["total_dv_m_s_yr"] for h in alts4]
...
if repeats:
    ...
    sc = ax.scatter(r_alts, r_opd, c=r_days, cmap="viridis_r",
...
fig6.suptitle("Orbital mechanics: J2 perturbations, frozen orbits, mission design",
              fontsize=FT+1, fontweight="bold")
...
track = propagate_ground_track(earth, 500_000, 98.0,
                                duration_s=3 * 86400, dt_s=120,
                                include_j2=True)
cov   = compute_coverage_map(earth, track, swath_width_km=120,
...
ax = axes7[0]; ax.set_facecolor("#E8F4FD")
for sp in ax.spines.values():
    sp.set_edgecolor("#444444"); sp.set_linewidth(0.8)
...
lats_s = lats[::step]; lons_s = lons[::step]; times_s = times[::step]

sc7 = ax.scatter(lons_s, lats_s, c=times_s, cmap="plasma",
                  s=0.8, alpha=0.8, zorder=3)
cb7 = fig7.colorbar(sc7, ax=ax, shrink=0.8, pad=0.03)
cb7.set_label("Time (hr)", fontsize=FA); cb7.ax.tick_params(labelsize=FA)
ax.set_xlim(-180, 180); ax.set_ylim(-90, 90)
ax.set_xticks(np.arange(-180, 181, 60)); ax.set_yticks(np.arange(-90, 91, 30))
ax.axhline(0, color="white", lw=0.5, alpha=0.5)
ax.set_xlabel("Longitude (°)", fontsize=FL); ax.set_ylabel("Latitude (°)", fontsize=FL)
ax.set_title(f"(a) 3-day ground track (500 km / 98°)", fontsize=FT, fontweight="bold", pad=3)
ax.tick_params(labelsize=FK)
...
im = ax.pcolormesh(lon_g, lat_g, np.minimum(cov.grid, 5),
                   cmap="Blues", shading="auto", vmin=0, vmax=5)
cb8 = fig7.colorbar(im, ax=ax, shrink=0.8, pad=0.03)
cb8.set_label("Observation count", fontsize=FA); cb8.ax.tick_params(labelsize=FA)
ax.set_xlim(-180, 180); ax.set_ylim(-90, 90)
ax.set_xlabel("Longitude (°)", fontsize=FL)
ax.set_ylabel("Latitude (°)", fontsize=FL)
ax.set_title(f"(b) Surface coverage ({cov.coverage_fraction()*100:.0f}% in 3 days, 120 km swath)",
             fontsize=FT, fontweight="bold", pad=3)
ax.tick_params(labelsize=FK)

fig7.suptitle("Ground track propagation and surface coverage analysis",
              fontsize=FT+1, fontweight="bold")
...
for col, (phase, label) in enumerate(seasons):
    ...
    T   = compute_temperature_map(ins, bond_albedo=0.3, emissivity=0.95,
                                   greenhouse_dT_K=25, thermal_inertia=800)
    T_insets.append(T)
    lats = ins.lat_deg; lons = ins.lon_deg
    lon_g, lat_g = np.meshgrid(lons, lats)

    ax_ins = axes8[0, col]; _ax(ax_ins)
    im1 = ax_ins.pcolormesh(lon_g, lat_g, ins.data_W_m2,
                             cmap="YlOrRd", shading="auto", vmin=0, vmax=500)
    cb = fig8.colorbar(im1, ax=ax_ins, shrink=0.85, pad=0.03)
    cb.set_label("W/m²", fontsize=FA); cb.ax.tick_params(labelsize=FA)
    ax_ins.set_title(f"(insolation) {label}", fontsize=FA, pad=2)
    ax_ins.set_xlabel("Lon (°)", fontsize=FA)
    if col == 0: ax_ins.set_ylabel("Lat (°)", fontsize=FA)
    ax_ins.tick_params(labelsize=FA-1)

    ax_T = axes8[1, col]; _ax(ax_T)
    im2 = ax_T.pcolormesh(lon_g, lat_g, T.data_K,
                           cmap="RdYlBu_r", shading="auto", vmin=210, vmax=320)
    cb2 = fig8.colorbar(im2, ax=ax_T, shrink=0.85, pad=0.03)
    cb2.set_label("K", fontsize=FA); cb2.ax.tick_params(labelsize=FA)
    ax_T.set_title(f"(temperature) {label}", fontsize=FA, pad=2)
    ax_T.set_xlabel("Lon (°)", fontsize=FA)
    if col == 0: ax_T.set_ylabel("Lat (°)", fontsize=FA)
    ax_T.tick_params(labelsize=FA-1)

    # Habitable zone contour (273-373 K)
    ax_T.contour(lon_g, lat_g, T.data_K, levels=[273, 373],
                 colors=["white"], linewidths=0.8, linestyles="--")

fig8.suptitle("Surface energy balance: insolation and temperature maps (Earth, obliquity 23.5°)",
              fontsize=FT+1, fontweight="bold")
...
ax.set_title("(a) Tidal heating vs distance (Io-like moon)", fontsize=FT, fontweight="bold", pad=3)
ax.legend(fontsize=FG-1, framealpha=0.9); ax.tick_params(labelsize=FK)

# (b) Tidal locking timescale vs orbital distance
ax = axes9[0, 1]; _ax(ax)
dists_lock = np.logspace(9.5, 12, 200)  # 3 to 10,000 AU in m
for star_obj, sc, sl in [(sun, W_ORANGE, "Sun"), (star_proxima_centauri(), W_RED, "Proxima")]:
    lock_times = [TidalLocking.locking_timescale_gyr(
                      6.371e6, 5.972e24, star_obj.mass, d)
                  for d in dists_lock]
...
for si, rho_sat in enumerate(sat_densities):
    rochef = [RocheLimit.fluid_satellite(p.radius, p.mean_density, rho_sat) / 1e3
              for p in roche_planets]
    ax.bar(x_pos + si*w, rochef, width=w, label=f"ρ_sat={rho_sat} kg/m³",
           color=WONG[si], edgecolor="white")
...
for planet, pc, pn in [(earth, W_BLUE, "Earth"), (mars, W_RED, "Mars")]:
    ...
    e_needed = [TidalHeating.equilibrium_eccentricity_for_target_flux(
...
ax.set_title("(d) Eccentricity for subsurface ocean", fontsize=FT, fontweight="bold", pad=3)
ax.legend(fontsize=FG-1, framealpha=0.9); ax.tick_params(labelsize=FK)

fig9.suptitle("Tidal dynamics: heating, locking, Roche limits, and ocean worlds",
              fontsize=FT+1, fontweight="bold")
...
ax = axes10[0, 0]; _ax(ax)
vinf_range = np.linspace(0.5, 8.0, 100)
for planet, pc, pn in [(earth, W_BLUE, "Earth"), (mars, W_RED, "Mars"),
                        (venus, W_ORANGE, "Venus")]:
    dvs = [orbital_insertion_dv(planet, v, 300)["dv_total_m_s"]/1e3
           for v in vinf_range]
...
ax = axes10[0, 1]; _ax(ax)
ab = plan_aerobraking(mars, initial_apoapsis_km=35000,
                       target_apoapsis_km=400, periapsis_altitude_km=115)
if ab.passes:
...
ax.set_title(f"(b) Mars aerobraking ({len(ab.passes)} passes)", fontsize=FT, fontweight="bold", pad=3)
ax.tick_params(labelsize=FK)
...
ax = axes10[0, 2]; _ax(ax)
mission_planets  = [(earth, 3.0, "Earth"), (mars, 2.5, "Mars"), (venus, 4.0, "Venus")]
budget_components = {}
for planet, vinf, pname in mission_planets:
    b = build_mission_dv_budget(planet, sun, planet.orbital_distance_m,
                                 vinf, 300, use_aerobraking=(planet==mars),
...
for ci, (comp_name, vals_dict) in enumerate(budget_components.items()):
    ...
    ax.bar(pnames, heights, bottom=bottom, color=WONG[ci % len(WONG)],
           edgecolor="white", label=comp_name[:22])
...
print(f"    Mars insertion (2.5 km/s): {orbital_insertion_dv(mars, 2.5, 300)['dv_total_m_s']:.0f} m/s")
ab2 = plan_aerobraking(mars, 35000, 400, 115)
print(f"    Mars aerobraking passes  : {ab2.total_passes}")
print(f"    Earth 400km drag life    : {DragLifetime.lifetime_years(earth, 400_000, 1000, 100):.1f} yr")
