...
fig1a, axes1a = plt.subplots(
    ...
    5,
    figsize=(7.08, 2.8),
    gridspec_kw=dict(wspace=0.05, left=0.01, right=0.99, top=0.83, bottom=0.04),
)
...
fig1b, axes1b = plt.subplots(
    ...
    figsize=(5.5, 8.5),
    gridspec_kw=dict(
        hspace=0.65, wspace=0.12, left=0.12, right=0.97, top=0.90, bottom=0.06
    ),
)
...
fig2a.suptitle(
    "Random planets — all features enabled", fontsize=FT + 1, fontweight="bold"
)
...
fig2b, axes2b = plt.subplots(
    ...
    figsize=(5.5, 10.0),
    gridspec_kw=dict(
        hspace=0.65, wspace=0.12, left=0.12, right=0.97, top=0.91, bottom=0.05
    ),
)
...
for row, planet in enumerate(randoms):
    plot_atmosphere_profile(
        planet,
        axes=[axes2b[row, 0], axes2b[row, 1], axes2b[row, 2]],
        max_altitude_km=150,
    )
...
for row, planet in enumerate(atm_planets):
    plot_atmosphere_profile(
        planet, axes=[axes3[row, 0], axes3[row, 1], axes3[row, 2]], max_altitude_km=180
    )

fig3.suptitle(
    "Atmosphere profiles by composition type", fontsize=FT + 1, fontweight="bold"
)
...
fig5.suptitle(
    "Feature toggle — identical planet seed", fontsize=FT + 1, fontweight="bold"
)
...
atm_d = np.array(
    [p.atmosphere.surface_density if p.atmosphere.enabled else 0.0 for p in batch]
)
...
fig6, axes6 = plt.subplots(
    ...
    figsize=(7.08, 4.5),
    gridspec_kw=dict(
        hspace=0.54, wspace=0.40, left=0.10, right=0.97, top=0.91, bottom=0.12
    ),
)
...
ax_m.bar(
    unique,
    counts,
    color=[WONG[i % len(WONG)] for i in range(len(unique))],
    edgecolor="white",
    lw=0.4,
    width=0.6,
)
