...
# Row 1: planet cross-sections (4 presets)
for idx, (name, factory) in enumerate(list(PRESETS.items())[:4]):
    ax = fig.add_subplot(3, 4, idx + 1)
    ax.set_facecolor(BG_DARK)
    plot_planet_cross_section(factory(), ax=ax)

# Row 2: atmosphere density profiles — one per planet using single-axis API
...
for idx, p in enumerate(atm_planets):
    ...
    plot_atmosphere_single(p, ax=ax, max_altitude_km=100, quantity="density")
    # Re-apply dark theme after function draws (it uses white defaults internally)
    ax.set_facecolor(BG_PANEL)
