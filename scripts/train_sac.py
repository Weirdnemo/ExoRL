...
parser.add_argument(
    "--planet", default="earth", help="Preset planet name (only used with --mode fixed)"
)
parser.add_argument(
    "--steps", type=int, default=200_000, help="Total environment steps"
)
parser.add_argument(
...
parser.add_argument(
    "--tag", default="run", help="Experiment name (used for output directory)"
)
...
parser.add_argument(
    "--pretrain", default=None, help="Path to BC-pretrained .zip model for warm start"
)
...
class MetricsCallback(BaseCallback):
    ...
    def __init__(
        self, out_dir: Path, eval_freq: int, eval_episodes: int, eval_env, verbose=1
    ):
    ...
    def _run_eval(self):
        ...
        line = (
            f"{self.num_timesteps},{mean_r:.4f},{std_r:.4f},"
...
try:
    ...
    if len(steps_l) > 1:
        ...
        fig.subplots_adjust(left=0.07, right=0.97, top=0.87, bottom=0.14, wspace=0.35)
