...
def plot_heliocentric_transfer(
    ...
    if standalone:
        fig, ax = plt.subplots(figsize=figsize,
                               gridspec_kw=dict(left=0.10, right=0.87,
    ...
    if additional_orbits:
        for r_m, name, col in additional_orbits:
            ...
            ax.text(r*0.707 + 0.03, r*0.707 + 0.03, name,
                    fontsize=FA-1, color="#999999")

    ax.plot(r_dep*np.cos(theta), r_dep*np.sin(theta),
            color=W_BLUE, lw=0.9, ls="-", alpha=0.4, zorder=2)
    ...
    lc = LineCollection(segs, cmap="plasma", norm=norm,
                        linewidth=1.8, zorder=4, alpha=0.95)
    ...
    ax.annotate("", xy=(mp[0]+mv_au[0], mp[1]+mv_au[1]),
                xytext=mp,
    ...
    if show_velocity_arrows:
        for pos, vel, col in [(dep_xy, traj[0,3:5], W_BLUE),
            ...
            dv = vel * scale
            ax.annotate("", xy=(pos[0]+dv[0], pos[1]+dv[1]),
                        xytext=pos,
    ...
    ax.legend(handles=handles, fontsize=FG, loc="lower left",
...
def plot_porkchop(porkchop_data,
    ...
    if standalone:
        fig, ax = plt.subplots(figsize=figsize,
                               gridspec_kw=dict(left=0.12, right=0.88,
    ...
    if show_tof_contours and quantity != "tof":
        ...
        if not np.all(np.isnan(tof_d)):
            ...
            ct    = ax.contour(DEP, ARR, tof_d, levels=lvls,
                                colors="#333333", linewidths=0.6,
    ...
    if best_window is not None:
        ...
        ax.text(best_window.departure_day + 8, best_window.arrival_day + 8,
    ...
    if agent_choice is not None:
        ...
        if 0 <= di < len(dep) and 0 <= ai < len(arr):
            ax.plot(dep[di], arr[ai], "D", ms=6, color=W_ORANGE,
                    markeredgecolor=W_BLACK, markeredgewidth=0.8, zorder=10)

    ax.set_xlabel("Departure day  (from epoch)", fontsize=FL)
    ax.set_ylabel("Arrival day  (from epoch)",   fontsize=FL)
    ax.tick_params(labelsize=FK)

    titles = {"c3": "Launch energy  (C3)",
