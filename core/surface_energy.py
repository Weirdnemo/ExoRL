...
def compute_insolation_map(planet,
    ...
    delta_sol = obliquity_deg * math.cos(2 * math.pi * orbital_phase)  # solar declination [°]
    ...
    for i, lat in enumerate(lats):
        ...
        if time_average:
            ...
            for j, lon in enumerate(lons):
                ...
                cos_z = (math.sin(phi) * math.sin(delta_rad)
                         + math.cos(phi) * math.cos(delta_rad) * math.cos(H))
...
def compute_temperature_map(insolation: InsolationMap,
    ...
    desc = (f"Surface temperature: albedo={bond_albedo}, TI={thermal_inertia:.0f}, "
...
def has_permanent_polar_ice(planet, star, orbital_distance_m: float,
                              obliquity_deg: float,
                              bond_albedo: float = 0.3) -> bool:
    """
...
def surface_energy_balance(planet,
                            star=None,
    ...
    if star is None or orbital_distance_m is None:
        raise ValueError("Provide star and orbital_distance_m, or set planet.star_context "
    ...
    if thermal_inertia is None:
        if planet.terrain.enabled:
            ...
            thermal_inertia = terrain_TI.get(
                planet.terrain.terrain_type.name, 400
            )
        else:
    ...
    for phase in [0.0, 0.25, 0.5, 0.75]:
        ins = compute_insolation_map(
            planet, S, obliquity_deg, phase,
            lat_res_deg, lon_res_deg, time_average=True
        )
    ...
    T_mean = compute_temperature_map(
        ins_mean, bond_albedo=bond_albedo, emissivity=0.95,
        greenhouse_dT_K=greenhouse_dT_K,
        thermal_inertia=thermal_inertia
    )

    # Solstice and equinox maps
    ins_sol = compute_insolation_map(planet, S, obliquity_deg, 0.0,
                                      lat_res_deg, lon_res_deg, True)
    T_sol   = compute_temperature_map(ins_sol, bond_albedo, 0.95,
                                       greenhouse_dT_K, thermal_inertia)

    ins_eq  = compute_insolation_map(planet, S, obliquity_deg, 0.25,
                                      lat_res_deg, lon_res_deg, True)
    T_eq_map = compute_temperature_map(ins_eq, bond_albedo, 0.95,
                                        greenhouse_dT_K, thermal_inertia)
