...
Usage
-----
    from core.science_ops_env import ScienceOpsEnv
...
class ScienceOpsEnv(gym.Env if GYM_OK else object):
    ...
    def _load_planet(self):
        from core.generator import PRESETS, PlanetGenerator
        from core.interior  import interior_from_bulk_density
        from core.star      import star_sun

        if self.randomize_planet:
            gen = PlanetGenerator(seed=self.generator_seed + self._episode_count)
            p = gen.generate(atmosphere_enabled=True, oblateness_enabled=True,
    ...
    def _compute_hab_tsm(self):
        ...
        try:
            ...
            ha = assess_habitability(
                self._planet, self._planet.star_context,
                self._planet.orbital_distance_m)
            self._hab_score = float(ha.overall_score)
        except Exception:
            self._hab_score = 0.3

        try:
            from core.observation import transmission_spectroscopy_metric
    ...
    def _build_resource_models(self):
        ...
        from core.power import PowerModel, SolarPanelConfig
        from core.comms import CommsModel, AntennaConfig

        panel = SolarPanelConfig(
            area_m2=self.panel_area, efficiency=0.28)
    ...
    def step(self, action: np.ndarray):
        ...
        new_alt = np.clip(self._alt_m + dalt_km * 1e3,
                          self.alt_min_m, self.alt_max_m)
        dv_alt = self._hohmann_dv(self._alt_m, new_alt)

        # Inclination change ΔV = 2 v_circ sin(Δi/2)
        new_inc = np.clip(self._inc_deg + dinc_deg,
                          DEFAULT_INC_MIN_DEG, DEFAULT_INC_MAX_DEG)
        ...
        if done:
            ...
            reward += self.r_data_bonus * (self._total_data_Gb
                       / max(self.buffer_max_Gb, 1.0))
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        obs[2]  = min(max(
                    ...
                    / max(self._power_model.peak_power_W, 1.0), -1.0), 1.0) * 0.5 + 0.5
