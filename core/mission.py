...
class DeltaVBudget:
    ...
    def report(self) -> str:
        ...
        for e in self.entries:
            cumulative += e["dv_m_s"]
            lines.append(
                f"  {e['name']:<30s} {e['dv_m_s']:>10.1f}  {e['description']}"
            )
        lines += [
            f"  {'─'*30}  {'─'*10}",
...
def orbital_insertion_dv(planet,
                          approach_vinf_km_s: float,
                          target_altitude_km: float,
                          periapsis_altitude_km: float = None) -> dict:
    """
    ...
    alt_peri = (periapsis_altitude_km * 1e3 if periapsis_altitude_km
                else alt_tgt)
...
def plan_aerobraking(planet,
                      ...
                      g_limit: float = 5.0,
                      max_passes: int = 500) -> AerobrakingCampaign:
    """
    ...
    if not planet.atmosphere.enabled:
        return AerobrakingCampaign(
            planet_name=planet.name, passes=[],
...
def lambert_solve(r1_vec: np.ndarray,
                   r2_vec: np.ndarray,
                   tof_s: float,
                   mu: float,
    ...
    T_min_energy = math.sqrt(2) / 3 * (s**1.5 - math.copysign((s-c)**1.5, lambda_val)) / math.sqrt(mu)
    ...
    for _ in range(50):
        ...
        if a > 0:
            ...
            T_x   = (y * (math.cos(psi) - lambda_val * math.sin(psi)) -
                     math.cos(psi) * math.sqrt(a)) / a
        else:
            psi   = (alpha - beta) / 2
            y     = math.sqrt(abs(1 - lam2 + lam2 * a))
            T_x   = (y * (math.cosh(psi) - lambda_val * math.sinh(psi)) -
                     math.cosh(psi) * math.sqrt(-a)) / (-a)
    ...
    r1_hat = r1_vec / r1
    r2_hat = r2_vec / r2
    t1_hat = np.cross(cross / np.linalg.norm(cross) if np.linalg.norm(cross) > 1e-10
                       else np.array([0,0,1]), r1_hat)
    t1_hat /= max(np.linalg.norm(t1_hat), 1e-15)
    t2_hat = np.cross(cross / np.linalg.norm(cross) if np.linalg.norm(cross) > 1e-10
                       else np.array([0,0,1]), r2_hat)
...
def porkchop_data(origin_star_mu: float,
    ...
    for i, t_dep in enumerate(departure_days):
        ...
        v1_vec  = np.array([-v1_circ * math.sin(theta1),
                              v1_circ * math.cos(theta1), 0.0])
        ...
        for j, t_arr in enumerate(arrival_days):
            ...
            v2_vec  = np.array([-v2_circ * math.sin(theta2),
                                  v2_circ * math.cos(theta2), 0.0])
...
class GravityAssist:
    ...
    @staticmethod
    def bending_angle(planet,
                       v_inf_km_s: float,
                       periapsis_altitude_km: float) -> float:
        """
    ...
    @staticmethod
    def outgoing_speed_km_s(incoming_speed_km_s: float,
                             bending_angle_rad: float,
                             planet_orbital_speed_km_s: float,
                             approach_angle_rad: float = 0.0) -> float:
        """
        ...
        v_out   = math.sqrt(
            v_inf**2 + v_p**2 + 2 * v_inf * v_p * math.cos(phi_out)
        )
        return v_out

    @staticmethod
    def summary(planet, v_inf_km_s: float,
                 periapsis_altitude_km: float) -> dict:
...
def build_mission_dv_budget(planet,
                              star,
                              ...
                              ballistic_coeff: float = 100.0) -> DeltaVBudget:
    """
    ...
    ins = orbital_insertion_dv(planet, approach_vinf_km_s,
                                target_altitude_km, periapsis_altitude_km)
    budget.add("Capture burn",
               ins["dv_capture_m_s"],
    ...
    budget.add(f"Station-keeping ({station_keeping_years:.0f} yr)",
               sk_total,
