...
from core.heliocentric import (LambertSolver, KeplerPropagator, HeliocentricIntegrator,
                                 HelioState, planet_state, MU_SUN, AU)
from core.soi import (SphereOfInfluence, HyperbolicArrival, laplace_soi_radius,
                       patched_conic_budget)
from core.launch_window import LaunchDecisionSpace, synodic_period_days, orbital_period_days
...
try:
    from core.star import STAR_PRESETS, star_sun
    _STAR_OK = True
except ImportError:
    _STAR_OK = False
    def star_sun():
        class _Sun:
            mass = 1.989e30; luminosity = 3.828e26; name = "Sun"
            hz_inner_m = 0.975*AU; hz_outer_m = 1.706*AU
            def flux_at_distance(self, d): return 1361*(AU/d)**2
            def orbital_period(self, r): return 2*math.pi*math.sqrt(r**3/(G*self.mass))
        return _Sun()
...
class InterplanetaryEnv(gym.Env if GYM_AVAILABLE else object):
    ...
    def _make_planet(self, name: str) -> Planet:
        ...
        if _STAR_OK:
            try:
                ...
                dist_map = {"earth": 1.000, "mars": 1.524, "venus": 0.723,
                            "moon": 1.000, "titan": 9.537}
    ...
    def _hab_score(self, planet: Planet) -> float:
        if (_HAB_OK
                and hasattr(planet, 'star_context') and planet.star_context
                and hasattr(planet, 'orbital_distance_m') and planet.orbital_distance_m):
            try:
                ha = assess_habitability(planet, planet.star_context,
    ...
    def reset(self, seed=None, options=None):
        ...
        syn_d = synodic_period_days(orbital_period_days(r_dep),
                                     orbital_period_days(r_arr))
        ...
        self._space = LaunchDecisionSpace(
            r_dep, r_arr,
            n_dep=self.n_dep, n_arr=self.n_arr,
    ...
    def _commit_window(self):
        ...
        if not cost['valid']:
            ...
            r_window = -self.r_window_scale * (cost['c3']/30.0 + cost['vinf_arr']/8.0)
    ...
    def _step_cruise(self, action):
        ...
        self._total_dv_used += (fuel_burned * self.Isp * 9.80665
                                / max(sc.mass_kg, 1.0))
    ...
    def _step_capture(self, action):
        ...
        self._sc_planet = SpacecraftState.from_array(
            arr_new, time=self._sc_planet.time + self.dt_capture,
            dry_mass=self.dry_mass)
        ...
        h   = float(np.linalg.norm(np.cross(
            self._sc_planet.position, self._sc_planet.velocity)))
        ecc = (math.sqrt(max(0.0, 1.0 + 2.0*eps*h**2/arr.mu**2))
               if arr.mu > 0 else 0.0)
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        if cost['valid']:
            ...
            obs[3] = 1.0; obs[4] = 1.0; obs[5] = 1.0; obs[6] = 0.0
        ...
        if self._sc_helio is not None:
            ...
            target_pos, _ = planet_state(cfg.arrival_radius_m,
                                          sc.time_s)
            ...
            # Angle to target
            if r > 0 and dist > 0:
                cos_a = float(np.dot(sc.position, target_pos)
        ...
        if self._sc_planet is not None and self._phase == PHASE_CAPTURE:
            ...
            rho = (arr.atmosphere.density_at_altitude(max(alt,0.0))
                   if arr.atmosphere.enabled else 0.0)
        ...
        if cfg is not None:
            ...
            obs[27] = min(arr.atmosphere.surface_pressure / 1e5
                          if arr.atmosphere.enabled else 0.0, 100.0)
    ...
    def _make_info(self) -> dict:
        phase_names = {PHASE_WINDOW: "window", PHASE_CRUISE: "cruise",
        ...
        info = {
            ...
            "fuel_remaining":   (self._sc_helio.fuel_kg if self._sc_helio else
                                 self._sc_planet.fuel_mass if self._sc_planet
