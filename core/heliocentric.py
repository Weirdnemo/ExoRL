...
class LambertSolver:
    ...
    def _stC(z: float) -> float:
        if abs(z) < 1e-6: return 0.5 - z/24 + z**2/720
        elif z > 0: return (1 - math.cos(math.sqrt(z))) / z
        else: return (math.cosh(math.sqrt(-z)) - 1) / (-z)

    @staticmethod
    def _stS(z: float) -> float:
        if abs(z) < 1e-6: return 1/6 - z/120 + z**2/5040
        elif z > 0:
            sq = math.sqrt(z); return (sq - math.sin(sq)) / sq**3
        else:
            sq = math.sqrt(-z); return (math.sinh(sq) - sq) / sq**3

    def _t_of_z(self, z: float, r1: float, r2: float, A: float) -> Optional[float]:
        C = self._stC(z); S = self._stS(z)
        if C < 1e-12: return None
...
class KeplerPropagator:
    ...
    def propagate(self,
        ...
        if alpha > 1e-6:
            ...
            # Hyperbolic
            a    = 1.0 / alpha
            chi0 = (math.copysign(1.0, dt) * math.sqrt(-a)
                    * math.log((-2.0 * self.mu * alpha * dt)
                               / (np.dot(r0, v0)
...
def planet_state(orbital_radius_m: float,
                  time_s: float,
    ...
    r_vec = np.array([
        orbital_radius_m * math.cos(theta),
        orbital_radius_m * math.sin(theta),
        0.0
    ])
    v_vec = np.array([
...
def planet_phase_at_epoch(orbital_radius_m: float,
                            reference_distance_m: float = AU,
                            reference_phase_rad: float = 0.0,
                            mu_star: float = MU_SUN) -> float:
    """
...
class HelioState:
    ...
    def eccentricity_vec(self) -> np.ndarray:
        ...
        r  = self.r
        mu = MU_SUN
        return ((self.v**2 - mu/r) * self.position
                - np.dot(self.position, self.velocity) * self.velocity) / mu
    ...
    @classmethod
    def from_array(cls, arr: np.ndarray, dry_mass: float,
                   time_s: float = 0.0) -> "HelioState":
        return cls(
            position    = arr[:3].copy(),
...
class HeliocentricIntegrator:
    ...
    def step(self,
        ...
        new_mass = max(state.dry_mass_kg,
                       m + (k1m + 2*k2m + 2*k3m + k4m) / 6.0)
...
def soi_radius(planet_mass_kg: float,
                orbital_radius_m: float,
                star_mass_kg: float = M_SUN) -> float:
    """
...
def is_in_soi(spacecraft_helio_pos: np.ndarray,
               planet_helio_pos:    np.ndarray,
               soi_radius_m:        float) -> bool:
...
def helio_to_planet_frame(spacecraft_helio_pos: np.ndarray,
...
def planet_to_helio_frame(spacecraft_planet_pos: np.ndarray,
...
def arrival_vinf(spacecraft_helio_vel: np.ndarray,
                  planet_helio_vel:    np.ndarray) -> float:
    """
...
def transfer_summary(departure_radius_m: float,
    ...
    if v1_sc is None:
        return TransferSummary(
            departure_planet=dep_name, arrival_planet=arr_name,
            tof_days=tof_s/86400, c3_km2_s2=float("nan"),
            vinf_dep_km_s=float("nan"), vinf_arr_km_s=float("nan"),
            dv_capture_m_s=float("nan"), dv_total_m_s=float("nan"),
            is_feasible=False,
        )
    ...
    if arrival_planet_mass and arrival_alt_m:
        ...
        r_peri  = (arrival_alt_m +
                   math.sqrt(arrival_planet_mass / 5.0e24) * 6.371e6)  # rough radius
