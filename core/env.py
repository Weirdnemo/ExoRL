...
try:
    from core.star import STAR_PRESETS, SpectralType
    from core.star import AU as _AU
    _STAR_OK = True
except ImportError:
    _STAR_OK = False
    _AU = 1.496e11
...
try:
    from core.orbital_analysis import FrozenOrbit, SunSynchronousOrbit
    _ORB_OK = True
except ImportError:
    _ORB_OK = False
...
class PlanetScienceContext:
    def __init__(self, planet: Planet, target_altitude_m: float,
        ...
        self.P_srf = planet.atmosphere.surface_pressure if planet.atmosphere.enabled else 0.0
        self.hab_score = 0.5
        if (_HAB_OK and hasattr(planet, 'star_context') and planet.star_context is not None
                and hasattr(planet, 'orbital_distance_m') and planet.orbital_distance_m):
            try:
                ha = assess_habitability(planet, planet.star_context, planet.orbital_distance_m)
                self.hab_score = float(ha.overall_score)
            except Exception:
                pass

        self.star_type_norm = 0.7
        if _STAR_OK and hasattr(planet, 'star_context') and planet.star_context is not None:
            mapping = {SpectralType.M: 0.0, SpectralType.K: 0.3, SpectralType.G: 0.7, SpectralType.F: 0.85, SpectralType.A: 1.0}
            self.star_type_norm = float(mapping.get(planet.star_context.spectral_type, 0.7))
        ...
        if _ORB_OK and self.j2 > 0:
            try:
                fe = FrozenOrbit.frozen_eccentricity(planet, planet.radius + target_altitude_m, math.radians(98.0))
        ...
        if _ORB_OK and self.j2 > 0:
            try:
                ss = SunSynchronousOrbit.sun_sync_inclination(planet, target_altitude_m, star_year_s)
...
class ScienceAtmosphere:
    def __init__(self, planet: Planet):
        ...
        if _ATM_OK and planet.atmosphere.enabled:
            try:
                self._multi = MultiLayerAtmosphere.from_atmosphere_config(planet.atmosphere, planet)
            except Exception:
                pass
...
def _attach_science_context(planet: Planet, rng: Optional[np.random.RandomState] = None) -> Planet:
    if _INTERIOR_OK and (not hasattr(planet, 'interior') or planet.interior is None):
        try:
            ...
            try:
                ...
                pass
    if _STAR_OK and (not hasattr(planet, 'star_context') or planet.star_context is None):
...
class CurriculumPool:
    def __init__(self, generator: PlanetGenerator, gen_kwargs: dict, pool_size: int = 200, easy_first: bool = True):
    ...
    def _build(self, generator, gen_kwargs, n):
        ...
        for _ in range(n):
            ...
            score = 0.5
            if (_HAB_OK and hasattr(p, 'star_context') and p.star_context
                    and hasattr(p, 'orbital_distance_m') and p.orbital_distance_m):
                try:
...
class OrbitalInsertionEnv(gym.Env if GYM_AVAILABLE else object):
    ...
    def __init__(
        ...
        initial_altitude: float = 100_000,
        initial_speed_ratio: float = 1.1, # Eased for better early exploration
        ...
        self.gen_kwargs = dict(
            atmosphere_enabled=atmosphere_enabled, terrain_enabled=terrain_enabled,
            magnetic_field_enabled=magnetic_field_enabled, oblateness_enabled=oblateness_enabled, moons_enabled=moons_enabled,
        )
        ...
        self._curriculum_pool = CurriculumPool(self.generator, self.gen_kwargs, pool_size=curriculum_pool_size, easy_first=curriculum_easy_first) if curriculum_mode and randomize_planet else None
        ...
        if GYM_AVAILABLE:
            # Goal-Conditioned Dict Space for HER
            self.observation_space = spaces.Dict({
                "observation": spaces.Box(low=-np.inf, high=np.inf, shape=(self.obs_dim,), dtype=np.float32),
                "achieved_goal": spaces.Box(low=-np.inf, high=np.inf, shape=(2,), dtype=np.float32), # [alt_norm, ecc]
                "desired_goal": spaces.Box(low=-np.inf, high=np.inf, shape=(2,), dtype=np.float32),  # Target [1.0, 0.0]
            })
            self.action_space = spaces.Box(low=np.array([-1.0, -1.0, -1.0], dtype=np.float32), high=np.array([ 1.0,  1.0,  1.0], dtype=np.float32), dtype=np.float32)

    def _select_planet(self) -> Planet:
        if self._fixed_planet is not None: return self._fixed_planet
        if self._curriculum_pool is not None: return self._curriculum_pool.next()
        if self.planet_preset and self.planet_preset in PRESETS: p = PRESETS[self.planet_preset]()
        elif self.randomize_planet: p = self.generator.generate(**self.gen_kwargs)
        else: p = PRESETS["earth"]()

        if self.attach_star: p = _attach_science_context(p, self.generator.rng)
        if self.use_science_j2 and p.oblateness.enabled and hasattr(p, 'derived_J2'):
            try:
                j2 = float(p.derived_J2())
                if j2 > 0: p.oblateness.J2 = j2
    ...
    def _get_obs(self) -> dict:
        ...
        atm_density = self._sci_atm.density_at(alt) if self._sci_atm is not None else (p.atmosphere.density_at_altitude(max(alt, 0.0)) if p.atmosphere.enabled else 0.0)
        ...
        alt_norm = alt / (self.target_altitude + 1e-9)
        
        obs_core = np.array([
            alt_norm, spd / (v_target + 1e-9), fpa / math.pi, min(ecc, 2.0),
            float(fuel_frac), min(sc.heat_load / (self.heat_limit + 1e-9), 1.0),
            p.radius / 6.371e6, p.surface_gravity / 9.81, min(atm_density / 1.225, 10.0),
            self.target_altitude / (p.radius + 1e-9),
        ], dtype=np.float32)

        if self.obs_dim > 10:
            obs_sci = self._sci_ctx.to_obs_slice() if self._sci_ctx is not None else np.zeros(8, dtype=np.float32)
            obs_core = np.concatenate([obs_core, obs_sci])

        # Formulate Goal-Conditioned Dict
        achieved_goal = np.array([alt_norm, min(ecc, 2.0)], dtype=np.float32)
        desired_goal = np.array([1.0, 0.0], dtype=np.float32) # Target is 1.0x target altitude, 0.0 eccentricity

        return {
            "observation": obs_core,
            "achieved_goal": achieved_goal,
            "desired_goal": desired_goal
        }

    def compute_reward(self, achieved_goal: np.ndarray, desired_goal: np.ndarray, info: dict) -> float:
        """Sparse reward required by HER and GoalEnv"""
        # compute_reward can be called with batches by HER
        if len(achieved_goal.shape) == 1:
            achieved_goal = achieved_goal.reshape(1, -1)
            desired_goal = desired_goal.reshape(1, -1)
            
        alt_err = np.abs(achieved_goal[:, 0] - desired_goal[:, 0])
        ecc_err = achieved_goal[:, 1]
        
        # Sparse reward: 0 if within tolerances, -1 otherwise
        success = np.logical_and(alt_err < 0.05, ecc_err < 0.05)
        rewards = np.where(success, 0.0, -1.0)
        
        return float(rewards[0]) if len(rewards) == 1 else rewards

    def reset(self, seed: Optional[int] = None, options=None):
        if seed is not None: self.generator.rng.seed(seed)
        ...
        if self.obs_dim >= 18:
            ...
            if hasattr(self.planet, 'star_context') and self.planet.star_context is not None and hasattr(self.planet, 'orbital_distance_m') and self.planet.orbital_distance_m:
                try: star_yr = self.planet.star_context.orbital_period(self.planet.orbital_distance_m)
                except Exception: pass
            self._sci_ctx = PlanetScienceContext(self.planet, self.target_altitude, star_yr)
        else:
            self._sci_ctx = None

        self.integrator = OrbitalIntegrator(planet=self.planet, thruster=ThrusterConfig(max_thrust=self.max_thrust, Isp=self.Isp), aero=AeroConfig(enabled=self.planet.atmosphere.enabled))
        ...
        self.state = SpacecraftState(x=self.planet.radius + approach_alt, y=0.0, z=0.0, vx=0.0, vy=v0, vz=0.0, mass=self.wet_mass, dry_mass=self.dry_mass)
        self._step_count   = 0
        self._initial_fuel = self.wet_mass - self.dry_mass
        self._trajectory   = [self.state]

        info = {"planet": self.planet.name, "habitability": self._sci_ctx.hab_score if self._sci_ctx else 0.0}
    ...
    def step(self, action: np.ndarray):
        ...
        thrust_vec = t_mag * (math.cos(pitch) * math.cos(yaw) * t_hat + math.cos(pitch) * math.sin(yaw) * n_hat + math.sin(pitch) * r_hat)
        ...
        arr_new = self.integrator.step_rk4(self.state.to_array(), thrust_vec, self.dt, self.state.time)
        self.state = SpacecraftState.from_array(arr_new, time=self.state.time + self.dt, dry_mass=self.dry_mass)

        if _orig is not None: self.planet.atmosphere.density_at_altitude = _orig
        ...
        # Delegate reward calculation to the HER-compatible method
        reward = self.compute_reward(obs_dict["achieved_goal"], obs_dict["desired_goal"], {})
        success = reward == 0.0 # Success is implicitly when sparse reward == 0

        terminated = crashed or escaped or overheated or success
        truncated  = timeout or no_fuel

        # Add heavy penalty only for absolute failure states
        if crashed or escaped: reward -= 50.0 

        info = {
            "altitude_m":   alt,
            "speed_mps":    self.state.speed,
            "eccentricity": float(obs_dict["achieved_goal"][1]),
            "fuel_kg":      self.state.fuel_mass,
            "fuel_fraction": float(obs_dict["observation"][4]), # Fixed logging bug!
        ...
        return obs_dict, reward, bool(terminated), bool(truncated), info