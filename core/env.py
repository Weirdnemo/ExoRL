...
class PlanetScienceContext:
    ...
    def __init__(self, planet: Planet, target_altitude_m: float,
        ...
        if (_STAR_OK
                ...
                and planet.star_context is not None):
            mapping = {
                SpectralType.M: 0.0, SpectralType.K: 0.3,
                SpectralType.G: 0.7, SpectralType.F: 0.85,
                SpectralType.A: 1.0,
            }
...
def _attach_science_context(planet: Planet,
    ...
    if _STAR_OK and (not hasattr(planet, 'star_context') or planet.star_context is None):
        ...
        try:
            ...
            if rng is not None:
                ...
                planet.orbital_distance_m = float(
                    hz_mid + rng.uniform(-spread, spread))
            else:
...
class CurriculumPool:
    ...
    def __init__(self, generator: PlanetGenerator, gen_kwargs: dict,
                 pool_size: int = 200, easy_first: bool = True):
    ...
    def _build(self, generator, gen_kwargs, n):
        ...
        for _ in range(n):
            ...
            score = 0.5
            if (_HAB_OK
                    and hasattr(p, 'star_context') and p.star_context
                    and hasattr(p, 'orbital_distance_m') and p.orbital_distance_m):
                try:
                    ha = assess_habitability(p, p.star_context,
                                              p.orbital_distance_m)
...
class OrbitalInsertionEnv(gym.Env if GYM_AVAILABLE else object):
    ...
    def __init__(
        ...
        if curriculum_mode and randomize_planet:
            self._curriculum_pool = CurriculumPool(
                self.generator, self.gen_kwargs,
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        obs_sci = (self._sci_ctx.to_obs_slice()
                   if self._sci_ctx is not None
                   else np.zeros(8, dtype=np.float32))
        return np.concatenate([obs_core, obs_sci])

    # ── Reward ────────────────────────────────────────────────────────────────
    def _compute_reward(self, obs: np.ndarray, done: bool,
                         success: bool, crashed: bool) -> float:
        """
        ...
        """
        dv_curr = float(obs[1]) * self._dv_init   # spd_norm * dv_init is approx dv_remaining
        ...
        total = shaped + progress + alt_proximity - self.reward_fuel_weight * (1.0 - float(obs[4]))
    ...
    def reset(self, seed: Optional[int] = None, options=None):
        ...
        if self.obs_dim >= 18:
            ...
            if (hasattr(self.planet, 'star_context')
        ...
        self.state = SpacecraftState(
            x=self.planet.radius + approach_alt, y=0.0, z=0.0,
            vx=0.0, vy=v0, vz=0.0,
        ...
        info = {
            ...
            "star":         (self.planet.star_context.name
                             if hasattr(self.planet, 'star_context')
                             and self.planet.star_context else "none"),
            "atm_model":    ("multi-layer" if self._sci_atm and self._sci_atm._multi
                             else "exponential"),
        }
        return obs, info
