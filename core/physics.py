...
class SpacecraftState:
    ...
    def to_array(self) -> np.ndarray:
        return np.array([self.x, self.y, self.z, self.vx, self.vy, self.vz,
                         self.mass, self.heat_load])

    @classmethod
    def from_array(cls, arr: np.ndarray, time: float = 0, dry_mass: float = 300) -> "SpacecraftState":
        return cls(x=arr[0], y=arr[1], z=arr[2],
                   vx=arr[3], vy=arr[4], vz=arr[5],
                   mass=arr[6], heat_load=arr[7],
                   time=time, dry_mass=dry_mass)

    @classmethod
    def circular_orbit(cls, planet: Planet, altitude: float,
                       inclination: float = 0.0,
                       wet_mass: float = 1000.0, dry_mass: float = 300.0,
                       **kwargs) -> "SpacecraftState":
        ...
        return cls(
            x=r, y=0, z=0,
            vx=0, vy=v * math.cos(inc), vz=v * math.sin(inc),
            mass=wet_mass, dry_mass=dry_mass, **kwargs
        )
