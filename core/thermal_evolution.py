...
Usage
-----
    from core.thermal_evolution import ThermalEvolution
...
class ThermalEvolution:
    ...
    def __init__(self, planet, star=None, orbital_dist: float = None,
                 age_gyr: float = None):
        self.planet       = planet
        ...
        self.orbital_dist = (orbital_dist
                             ...
                             or AU)
        self.age_gyr      = age_gyr or getattr(
            getattr(self.star, None, None), "age", 4.5) or 4.5
        ...
        self.P_atm_0   = max(float(getattr(planet.atmosphere, 'surface_pressure', 101325)),
                              P_MIN_PA)
    ...
    def _radiogenic_flux(self, age_gyr: float) -> float:
        ...
        q_radio_now = (49e-3 * (self.M / M_EARTH) ** 0.7
                       * 0.44)   # 44% of total is radiogenic at 4.5 Gyr
    ...
    def _secular_flux(self, age_gyr: float) -> float:
        ...
        q_sec_now = (Q_SEC_EARTH * (self.M / M_EARTH) ** 0.7
    ...
    def _greenhouse_warming(self, P_atm_Pa: float, T_eq: float,
                             comp_name: str = "EARTH_LIKE") -> float:
    ...
    def run(self, dt_myr: float = 10.0, bond_albedo: float = 0.3) -> list[ThermalSnapshot]:
        """
        ...
        P_atm   = self.P_atm_0
        comp_nm = getattr(
            getattr(self.planet, "atmosphere", None), "composition", None
        )
        comp_name = comp_nm.name if comp_nm and hasattr(comp_nm, "name") else "EARTH_LIKE"
        ...
        for age in ages:
            ...
            from core.interior import DYNAMO_MAX_PERIOD_HR
            ...
            dP_escape = (ESCAPE_COEFF * xuv * dt_gyr * 1000
                         * (P_atm / 101325) / shield)  # stronger at higher P
    ...
    def dynamo_lifetime_gyr(self) -> float:
        ...
        return sum(s.dynamo_active for s in self.history) * (
            ...
            if len(self.history) > 1 else 0.01)
    ...
    def habitable_duration_gyr(self) -> float:
        ...
        if not self.history:
            return 0.0
        dt = (self.history[1].age_gyr - self.history[0].age_gyr
              if len(self.history) > 1 else 0.01)
    ...
    def plot(self, figsize=(10.0, 7.0), title: str = None):
        ...
        from visualization.visualizer import (W_BLUE, W_RED, W_GREEN, W_ORANGE,
                                               W_BLACK, FT, FL, FK, FA, LW, _ax)
        ...
        # (a) Heat flux
        ax = fig.add_subplot(gs[0, 0]); _ax(ax)
        ax.fill_between(t, arr["q_radio"], alpha=0.3, color=W_RED,   label="Radiogenic")
        ax.fill_between(t, arr["q_radio"], arr["heat_flux"],
                        alpha=0.3, color=W_BLUE, label="Secular")
        ...
        ax = fig.add_subplot(gs[0, 1]); _ax(ax)
        dynamo_on = arr["dynamo"].astype(float)
        ax.plot(t, arr["T_core"] / 1000, color=W_ORANGE, lw=1.4)
        ax.axhline(T_CORE_SOLIDUS / 1000, color='#888888',
                   lw=0.8, ls='--', alpha=0.7)
        ax.text(t[1], T_CORE_SOLIDUS/1000 * 1.01, "Solidification", 
                fontsize=FA-1.5, color='#666666')
        # Shade dynamo-active region
        ax.fill_between(t, 0, arr["T_core"]/1000,
                        where=arr["dynamo"], alpha=0.12,
                        color=W_BLUE, label="Dynamo active")
        ax.set_xlabel("Age  (Gyr)", fontsize=FL)
        ax.set_ylabel("Core temperature  (10³ K)", fontsize=FL)
        ax.legend(fontsize=FA-1, frameon=False)
        ax.set_title("(b) Core temperature & dynamo", fontsize=FT, fontweight="bold", pad=4)
        ax.tick_params(labelsize=FK)

        # (c) Atmospheric pressure
        ax = fig.add_subplot(gs[1, 0]); _ax(ax)
        ax.semilogy(t, arr["P_atm_bar"], color=W_GREEN, lw=1.4)
        ax.axhline(6e-3, color='#888888', lw=0.8, ls='--', alpha=0.7)
        ax.text(t[1], 6e-3*1.2, "Water triple point", fontsize=FA-1.5, color='#666666')
        ax.set_xlabel("Age  (Gyr)", fontsize=FL)
        ...
        ax = fig.add_subplot(gs[1, 1]); _ax(ax)
        ax.plot(t, arr["T_eq"],   color=W_ORANGE, lw=1.0, ls='--',
                alpha=0.7, label="T_eq (no GH)")
        ax.plot(t, arr["T_surf"], color=W_BLACK,  lw=1.4, label="T_surf")
        ax.fill_between(t, 273, 373, alpha=0.08, color=W_GREEN,
                        label="Liquid water range")
        # Shade habitable intervals
        ax.fill_between(t, t*0+240, t*0+390, where=arr["habitable"],
                        alpha=0.25, color=W_GREEN)
        ax.set_xlim(t[0], t[-1])
        ax.set_ylim(100, 500)
        ...
        fig.suptitle(
            title or f"Thermal evolution:  {self.planet.name}  "
