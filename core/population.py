...
def classify_composition(mass_earth: float,
                           radius_earth: float) -> str:
    """
...
class PlanetPopulation:
    ...
    def generate(cls,
        ...
        from core.observation import transit_depth_ppm as tdppm, rv_semi_amplitude
        ...
        for i in range(n):
            ...
            try:
                ...
                has_dynamo  = planet.interior.dynamo_active(
                    planet.radius, planet.mass) if planet.interior else False
            except Exception:
            ...
            if planet.atmosphere.enabled:
                ...
                if _atm_ok and star:
                    try:
                        aa = analyse_atmosphere(planet, star,
            ...
            if star:
                try:
                    ha        = assess_habitability(planet, star,
                                                     planet.orbital_distance_m)
                    hab_score = float(ha.overall_score)
                    hab_grade = ha.grade
                    in_hz     = bool(star.in_habitable_zone(
                        planet.orbital_distance_m))
                except Exception:
                    pass

            # Composition
            comp = classify_composition(planet.mass / M_EARTH,
            ...
            if star:
                try:
                    ...
                    T_orb  = 2*math.pi*math.sqrt(
                        planet.orbital_distance_m**3 / mu_s)
                    rv_K   = float(rv_semi_amplitude(
                        planet.mass, star.mass, T_orb))
                except Exception:
                    pass

            records.append(PlanetRecord(
                index              = i,
                name               = planet.name,
    ...
    def correlation_matrix(self) -> tuple[np.ndarray, list[str]]:
        ...
        """
        a = self.arrays()
        keys = ["mass", "radius", "density", "gravity", "j2", "b_field",
                "heat_flux", "moi", "P_srf", "T_surf", "hab_score",
