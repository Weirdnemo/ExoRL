...
parser.add_argument("--out",         default="demos/demos.npz",
...
parser.add_argument("--speed-ratio", type=float, default=1.08,
...
if not args.quiet:
    ...
    print(f"  obs_dim={args.obs_dim}  speed_ratio={args.speed_ratio}  "
          f"max_steps={args.max_steps}")
    print(f"  {'presets only' if args.presets_only else 'presets + random (50/50)'}")
    print(f"  Output: {args.out}\n")
    print(f"  {'ep':>5}  {'planet':<12}  {'ok':>4}  {'steps':>6}  "
...
for ep, (preset, seed) in enumerate(planet_schedule(args.episodes, args.presets_only)):
    ...
    if not args.quiet and (ep % 25 == 0 or ep < 5):
        ...
        print(f"  {ep+1:5d}  {planet_name:<12}  "
...
if not args.quiet:
    print(f"\n{'─'*57}")
    print(f"  Episodes:   {n_total}  ({n_success} successful, "
          f"{n_success/n_total*100:.0f}%)")
    print(f"  Pairs (obs,act): {len(obs_arr):,}  "
          f"(from successful episodes only)")
