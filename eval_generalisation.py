...
parser.add_argument("--out",      default=None,
                    help="Output path (default: next to model file)")
args = parser.parse_args()

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import core.planet_io
...
def evaluate_on_planet(planet_name_or_obj, n_episodes, policy,
                       is_preset=True, planet_fingerprint=None):
    """
    ...
    return {
        ...
        "fuel_efficiency_vs_hohmann": (
            ...
            if hohmann_fuel_frac and hohmann_fuel_frac > 0 else None
        ),
    }
...
header = (f"{'Planet':<10}  {'Success':>8}  {'Reward':>10}  "
...
if args.random_baseline:
    ...
    random_results = []
    for planet_name in args.planets:
        r = evaluate_on_planet(planet_name, args.episodes // 2,
                               "random", is_preset=True)
        random_results.append(r)
        print(f"  {planet_name:<10}  {r['success_rate']:>7.1%}  "
              f"{r['mean_reward']:>10.1f}")
    ...
    for entry in candidates:
        try:
            ...
            r  = evaluate_on_planet(
                planet_obj, args.episodes // 2,
                model, is_preset=False,
                planet_fingerprint=fp
            )
            r["planet"] = entry.name
            results["kepler_results"].append(r)
            print(f"  {entry.name:<22}  success={r['success_rate']:>6.1%}  "
...
try:
    ...
    bars = axes[0].bar(x, success_rates, color=BLUE, alpha=0.85, zorder=3,
    ...
    bars2 = axes[1].bar(x, mean_rewards, color=ORANGE, alpha=0.85, zorder=3,
                        edgecolor="white", lw=0.5)
    axes[1].set_xticks(x)
    ...
    if args.random_baseline and "random_baseline" in results:
        ...
        axes[0].plot(x, rand_sr, "o--", color="#CC3311", ms=5, lw=1.2,
                     label="Random policy")
    ...
    fig.savefig(str(out_dir / "generalisation_table.png"), dpi=150,
                bbox_inches="tight")
    plt.close(fig)
