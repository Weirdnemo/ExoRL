...
parser.add_argument("--out",    type=str, default="science_figures", help="Output directory")
...
parser.add_argument("--save-csv", action="store_true", default=True,
                    help="Save population data to CSV")
...
print(f"  Gravity range (5-95th pct):  {stats['gravity_p5']:.1f} – {stats['gravity_p95']:.1f} m/s²")
print(f"  Radius range  (5-95th pct):  {stats['radius_p5']:.2f} – {stats['radius_p95']:.2f} R⊕")
print(f"  Mass range    (5-95th pct):  {stats['mass_p5']:.3f} – {stats['mass_p95']:.1f} M⊕")
print(f"  Hab score mean / std:         {stats['hab_score_mean']:.3f} / {stats['hab_score_std']:.3f}")
